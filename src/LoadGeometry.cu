    ...
    const std::filesystem::path& sceneFilePath, const aiScene** sceneCachePtr)
{
    ...
    if (!fileExists) {
        errStream<<"File:"<<sceneFilePath<<" could not be found."<<std::endl;
        throw std::exception(errStream.str().c_str());
    }
    auto sceneCache = aiImportFile(sceneFilePath.u8string().c_str(), aiProcess_Triangulate);

    //Assimp::Importer importer;
    //auto sceneCache = importer.ReadFile(sceneFilePath.string().c_str(), aiProcess_Triangulate);
    *sceneCachePtr = sceneCache;

    if (!sceneCache)
    {
        errStream<<"Could not open file: "<<sceneFilePath<<" Reason: "<<aiGetErrorString()<<std::endl;
        //errStream<<"Could not open file: "<<sceneFilePath<<" Reason: "<<importer.GetErrorString()<<std::endl;
        throw std::exception(errStream.str().c_str());
    }
...
//
    //    for (uint n=0 ; n < numMeshes; ++n) {
    //        auto mesh = sceneCache->mRootNode->mMeshes[sceneCache->mRootNode->mMeshes[n]];
    //        assert(mesh != nullptr);
