...
public:
    MacGrid(int _gridSize_i, int _gridSize_j, int _gridSize_k) :
        gridSize_i(_gridSize_i),
        gridSize_j(_gridSize_j),
        gridSize_k(_gridSize_k),
        cellNum(gridSize_i * gridSize_j * gridSize_k) {
        std::cout<<cellNum<<std::endl;
        cudaMalloc(&d_pressureBuff, cellNum * sizeof(float));
    }

    void getPressure(float * h_pressureBuff) {
        int error = cudaMemcpy(h_pressureBuff, d_pressureBuff, cellNum * sizeof(float), cudaMemcpyDeviceToHost);
...
void gridPressureTest() {
    ...
    //initPressureArray<<<0, grid.cellNum>>>(grid.d_pressureBuff, grid.cellNum, 5);
